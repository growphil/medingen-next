  ...
  if (isMobileView && hideHeader) {
    ...
    const renderOptions = () => {
      switch (activeMobileTab) {
          ...
          return subCategoryOptions.length > 0 ? (
            subCategoryOptions.map((option, idx) => {
              ...
              return (
                <div
                  key={`${name}-${idx}`}
                  className={`filter-radio-item ${filters.subCategories.includes(name) ? 'selected' : ''}`}
                  onClick={() => handleCheckboxChange('subCategories', name)}
                >
                  <span className="filter-radio-label">{name}</span>
        ...
        case "Form":
          return dosageOptions.map((option, idx) => (
            <div
              key={`${option}-${idx}`}
            ...
            <div className="filter-section-body">
              {subCategoryOptions.length > 0 ? (
                subCategoryOptions.map((option, idx) => {
                  const name = typeof option === 'object' ? option.name : option;
                  return (
                    <label key={`${name}-${idx}`} className="filter-checkbox-item">
                      <input
                        type="checkbox"
          ...
          {sectionsExpanded.dosageForm && (
            <div className="filter-section-body">
              {dosageOptions.map((option, idx) => (
                <label key={`${option}-${idx}`} className="filter-checkbox-item">
                  <input
                    type="checkbox"
