...
const ProductSection = ({ title, categoryKey }) => {
    ...
    const updateInlineQuantity = async (productId, delta) => {
        try {
            const quantitiesMap = {};
                ...
                >
                    {products.map((product, idx) => {
                        ...
                        const cartItem = cartItems?.find(item => {
                          ...
                          const currentPid = String(product.product_id || product.id || "");
                          
                          const cartName = (item.productName || item.product_name || item.name || "").toLowerCase().trim();
                          const currentName = (product.productName || product.product_name || product.name || "").toLowerCase().trim();
