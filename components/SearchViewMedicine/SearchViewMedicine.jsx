...
const SubstitutionComparison = ({ 
  ...
  const productImages = recommendedProduct.slides || [
    recommendedProduct.imageUrl?.startsWith("http") ? recommendedProduct.imageUrl : `https://d1dh0rr5xj2p49.cloudfront.net/products/${recommendedProduct.imageUrl}`
  ];
...
export default function SearchViewMedicine() {
  ...
  const rawSlug = decodeURIComponent(location.pathname.replace("/product/", ""));
  const suggestion = useMemo(() => state?.product || { productName: rawSlug }, [state?.product, rawSlug]);

  const [product, setProduct] = useState(null);
  const [medicineDetails, setMedDetails] = useState({});
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => { console.warn("Error obtaining location:", error.message); },
  ...
  const fetchProduct = useCallback(async () => {
    ...
    try {
      ...
      const imgList = (result.images || []).map((img) => ({ url: `https://d1dh0rr5xj2p49.cloudfront.net/products/${img.img}` }));
      setSlides(imgList);
              ...
              .map((x) => {
                ...
                try {
                  ...
                  if (Array.isArray(parsed)) {
                    imgs = parsed.map(i => i.img).map(u => u.startsWith("http") ? u : "https://d1dh0rr5xj2p49.cloudfront.net/products/" + u);
                  }
                } catch (e) { }

                if (!imgs.length && x.first_image_url) {
                  imgs = [x.first_image_url.startsWith("http") ? x.first_image_url : "https://d1dh0rr5xj2p49.cloudfront.net/products/" + x.first_image_url];
                }
  ...
  const recommendedAlt = (() => {
    ...
    if (product.rc === 0 && product.recommended_products?.length > 0) {
      ...
      try {
        ...
        if (Array.isArray(parsed)) imgs = parsed.map(i => i.img).map(u => u.startsWith("http") ? u : "https://d1dh0rr5xj2p49.cloudfront.net/products/" + u);
      } catch (e) {}
      rec = {
        ...
        <div style={{ maxWidth: 1280, margin: isMobile ? "32px auto 0" : "24px auto 0", padding: isMobile ? "0 20px" : "0 32px" }}>
          ...
          <div ref={freqScrollRef} style={{ display: "flex", gap: 16, overflowX: "auto", paddingBottom: 12, scrollbarWidth: "none" }}>
            {freqMeds.map((p, i) => {
              ...
              const img = p.images?.length > 0 ? `https://d1dh0rr5xj2p49.cloudfront.net/products/${p.images[0].img}` : "/medicine-details.png";
              ...
              return (
                <div key={blog.id || i}
                  ...
                  <div style={{ height: isMobile ? 165 : 185, overflow: "hidden", background: "#f8f9fa", position: "relative" }}>
                    <img src={`https://d1dh0rr5xj2p49.cloudfront.net/blogs/images/${blog.blog_image_url}`} alt={blog.blog_name}
  ...
  const renderMedicineContent = () => {
    if (!product) return null;
    const hasPremiumContent = (product.productDescription || (product.benefits?.length > 0) || (product.drugInteractions?.length > 0) || product.howItWorks || (product.howToUse?.length > 0) || (product.safetyAdvice?.length > 0) || (product.sideEffects?.length > 0) || (product.faq?.length > 0) || product.references);
                    ...
                    return (
                      <div key={i} className="sv-product-card" style={{ background: "#fff", borderRadius: 20, border: "1.5px solid #f3f4f6", padding: 16, display: "flex", flexDirection: "column", gap: 10, minWidth: 220, maxWidth: 220, flexShrink: 0, boxShadow: "0 2px 12px rgba(0,0,0,.04)", cursor: "pointer", transition: "transform .2s" }}
                        ...
                        <div style={{ position: "relative", background: "#f8f9fb", borderRadius: 12, height: 170, display: "flex", alignItems: "center", justifyContent: "center", padding: 12, overflow: "hidden" }}>
                          <img src={alt.imageUrl?.startsWith("http") ? alt.imageUrl : `https://d1dh0rr5xj2p49.cloudfront.net/products/${alt.imageUrl}`}
