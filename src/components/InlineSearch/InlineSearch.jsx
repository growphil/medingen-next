...
export const InlineSearch = ({ 
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            if (searchText.length > 2) {
                setLoading(true);
                setError("");
                // Removed setShowDropdown(true) to prevent auto-reopen on scroll

                try {
                    const saltresults = await searchSalt(searchText);
                    setFilteredSalts(saltresults || []);

                    const results = await searchProducts(searchText, currentPage);
                    if (results && results.results) {
                        const validProducts = results.results.filter(p => p.product_pricing_new !== null);
    ...
    return (
        <div className={`inline-search-container variant-${variant} ${className}`} ref={searchContainerRef}>
            <div className="inline-search-bar" onClick={() => { if (searchText.length > 2) setShowDropdown(true); }}>
                <input
                    type="text"
