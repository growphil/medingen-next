      ...
      try {
        ...
        Swal.fire({
          ...
          if (r.isConfirmed) {
            ...
            const cartResponse = await addToCart(null, prescriptionId, 0, navigate);
            if (!cartResponse) {
              Swal.fire("Error", "Failed to add prescription to cart", "error");
              return;
            }
            navigate("/cart");
          }
    ...
    Swal.fire({
      ...
      try {
        ...
        if (from === "request-product" && product) {
          ...
          const cartResponse = await addToCart(null, prescriptionId, 0, navigate);
          if (!cartResponse) throw new Error("Failed to add prescription to cart");
